...
public class UpdateTab extends JDialog {
    ...
    private void begin() {
        ...
        update.setLayout(new MigLayout("insets 2, fill", "[]", "[]2[]2[]2[]2[]2[]2[]2[]2[]"));
        ...
        update.add(automatic, "cell 0 1, grow"); //宽度已经固定为其上一级容器的宽度。拖放窗口可以看到标签自动换行。
        
        JLabel howToUpdate = new JLabel();
        howToUpdate.setForeground(Color.BLUE);
        howToUpdate.setText("<html><u>自动更新如何工作？</u>"); //无效：Windows Classic L&F 是不会显示下划线的。Bug。
...
//        howToUpdate.setFont(font.deriveFont(attributes));
        ...
        update.add(howToUpdate, "cell 0 2");
...
//        |                   | +-----------+  +----+  +----------+ |
//        +-------------------+-------------------------------------+
        green.setLayout(new MigLayout("insets 2, fill, debug", "[center]3[150!]3[50!]3[100!]", "[]3[30!]3[]"));//第二行无元素但必须空出来
        
        JLabel greenShield = new JLabel("");
        greenShield.setIcon(new ImageIcon(getClass().getResource("shield_green.gif")));
        green.add(greenShield, "cell 0 0 1 3, grow");
        
        JLabel downloadAndInstall = new JLabel("自动下载推荐的更新，并安装它们：");
        green.add(downloadAndInstall, "cell 1 0, span 3, grow"); //在第二列第一行显示，并跨3列
        ...
        update.add(onlyDownload, "cell 0 5, grow");
